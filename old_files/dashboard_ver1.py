...
dash_app = dash.Dash(__name__, server=app, url_base_pathname='/dashboard1/', external_stylesheets=external_stylesheets)

# Zdefiniowanie rozłożenia wykresów
dash_app.layout = html.Div([
    html.H1('Air Quality Dashboards - version 1', style={'textAlign': 'center'}),
    
    # Interval component do cyklicznych updateow wykresu
    dcc.Interval(
    ...
    # Side by side graphs of scatter and histogram
    html.Div([
        html.Div([
            html.H3('PM₁₀ and PM₂₅ scatter trend'),
            dcc.Graph(id='pm10_pm25_scatter_fig', style={'width': '100%', 'height': '400px'})
        ], style={'display': 'inline-block', 'width': '48%', 'padding': '1%'}),
        
        html.Div([
            html.H3('PM₁₀ and PM₂₅ histogram'),
            dcc.Graph(id='pm10_pm25_hist_fig', style={'width': '100%', 'height': '400px'})
        ], style={'display': 'inline-block', 'width': '48%', 'padding': '1%'}),
    ]),

    # O3, SO2, NO2 wskaźniki wykresów
    html.Div([
        html.Div([
            html.H3('O₃/CO trend'),
            dcc.Graph(id='o3_co_fig', style={'width': '100%', 'height': '400px'})
        ], style={'display': 'inline-block', 'width': '100%'})
    ]),

    # Heatmap level per day and sth else
    html.Div([
        html.Div([
            html.H3('SO₂/NO₂ trend'),
            dcc.Graph(id='so2_no2_fig', style={'width': '100%', 'height': '400px'})
        ], style={'display': 'inline-block', 'width': '48%', 'padding': '1%'}),
        
        html.Div([
            html.H3('AQI level heatmap'),
            dcc.Graph(id='aqi_level_fig', style={'width': '100%', 'height': '400px'})
        ], style={'display': 'inline-block', 'width': '48%', 'padding': '1%'}),
    ]),
...
def update_graphs(city, start_date, end_date):
    ...
    # Dodanie drugiej "warstwy" histogramu
    pm10_pm25_hist_fig.add_trace(px.histogram(pm_df, x='pm25', histfunc='count', opacity=0.75, 
                                              color_discrete_sequence=['orange']).data[0])


    # Utworzenie wykresów zawierających gazy
    gas_data = db.session.query(GasMeasurements.timestamp, GasMeasurements.co, GasMeasurements.no2, GasMeasurements.so2, GasMeasurements.o3) \
    ...
    gas_df = pd.DataFrame(gas_data, columns=['timestamp', 'co', 'no2', 'so2', 'o3'])
    gas_df = find_day_of_week(gas_df)


    # Wykres liniowy porównujący dwa gazy
    o3_co_fig = go.Figure()

    # Dodanie pozostałych wskaźników
    o3_co_fig.add_trace(go.Scatter(x=gas_df['timestamp'], y=gas_df['o3'], name='O₃'))
    o3_co_fig.add_trace(go.Scatter(x=gas_df['timestamp'], y=gas_df['co'], name='CO'))
    o3_co_fig.update_traces(mode='markers+lines')
    ...
    # Dodanie ścieżek
    so2_no2_fig.add_trace(go.Scatter(x=gas_df['timestamp'], y=gas_df['so2'], fill='tozeroy', mode='lines', name='SO₂', line=dict(color='green')))
    so2_no2_fig.add_trace(go.Scatter(x=gas_df['timestamp'], y=gas_df['no2'], fill='tozeroy', mode='lines', name='NO₂', line=dict(color='blue')))
    ...
    # Utworzenie heatmapy która przedstawia rozlozenie poziomu wsakźnika AQI na przestrzeni dni tygodnia
    ...
    aqi_df['day_of_week'] = pd.Categorical(aqi_df['day_of_week'], categories=['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'], ordered=True)
    ...
    aqi_level_fig = px.imshow(heatmap_data,
                labels=dict(x="Level", y="Day of Week", color="Count"),
                x=heatmap_data.columns,
                y=heatmap_data.index,
                color_continuous_scale='viridis')

    aqi_level_fig.update_layout(title='Distribution of AQI indicator levels across days of the week',
                    xaxis_title='Level',
